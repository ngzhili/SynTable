...
class Composer:
    ...
    def generate_scene(self, img_index, ann_index, img_list,ann_list):
        ...
        if amodal:
            ...
            # print(f"roomTableCenter:{[0,0,roomTableHeight]}")
        ...
        # zhili added, iteratively hide objects
            ...
            # print("\nList of objects:",[obj.name for obj in self.scene_manager.objs])
            print("\nNumber of Objects left in scene:", len(self.scene_manager.objs))
            # print('\n\nScene Manager:\n')
            ...
            # Randomly move light in hemispherical orbit
            for light in self.scene_manager.lights:
                print("Light Coords:",light.coord)
                light_coord_w = camera_orbit_coord(r=random.uniform(2.1,3),tableTopHeight=roomTableHeight+0.2)
                print("light_coord_w:",light_coord_w)
                ...
                # light.translate_rotate(target=(0,0,roomTableHeight)) #target coordinates
            
            # Generate camera coords constrainted between 2 concentric hemispheres above tabletop
            cam_pose_list = []
            numViews = self.params["num_views"]
            for _ in range(numViews):
                cam_coord_w = camera_orbit_coord(r=random.uniform(1,2),tableTopHeight=roomTableHeight+0.2)
                cam_pose_list.append(cam_coord_w)
            ...
            for view_id, cam_pose in enumerate(cam_pose_list):
                ...
                print(f"Setting camera coord to:{cam_pose}")
                self.scene_manager.camera.translate(cam_pose)
                ...
                #     img_list.append(img_info_list[i])
                #     ann_list.append([i])                   
...
if __name__ == "__main__":
    ...
    while index <= params["num_scenes"]:
        if index == 0:
            ...
            print(f'\nExecution time for all 100 scenes * 50 views:', time.strftime("%H:%M:%S", time.gmtime(elapsed_time)))
            ...
            del kit
            print(sys.modules)
            del sys.modules["omni.isaac.kit"]
    ...
    if params["save_segmentation_data"]:
        ...
        for scene_index in range(1,params["num_scenes"]*params["num_views"]+1):
            # scene_index = str(scene_index_raw) +"_"+str(view_id)
            rgb_img = cv2.imread(f"{rgb_dir}/{scene_index}.png", cv2.IMREAD_UNCHANGED)
            
            occ_img_list = glob.glob(f"{occ_dir}/{scene_index}_*.png")
            #occ_mask_list = []
            if len(occ_img_list) > 0:
                ...
                for i in range(len(occ_img_list)):
                    ...
                    if occ_mask_path == background:
                        ...
                        occ_save_path = f"{vis_occ_dir}/{scene_index}_background.png"
                ...
                occ_save_path = f"{vis_occ_dir}/{scene_index}_mask.png"
                cv2.imwrite(occ_save_path, combined_mask*255)


            vis_img_list = glob.glob(f"{instance_dir}/{scene_index}_*.png")
            if len(vis_img_list) > 0:
                vis_img = rgb_img.copy()
                overlay = rgb_img.copy()
                background = f"{instance_dir}/{scene_index}_background.png"
                # iterate through all occlusion masks
                for i in range(len(vis_img_list)):
                    ...
                    if vis_mask_path == background:
                        ...
                        vis_save_path = f"{vis_instance_dir}/{scene_index}_background.png"
